  ...
  <div class="theaters">
    ...
    <b-row id="row">
      ...
      <div id="cards">
        <b-card
          id="card"
          :title="theater.titulo"
          :img-src="theater.imagem"
          :img-alt="theater.descricao"
          img-top
          tag="article"
          style="max-width: 300px;"
          class="mb-4 text-center"
          :key="theater.id"
          v-for="theater in filteredList"
        >
          <b-card-text>{{theater.descricao}}</b-card-text>
          <b-card-text>R${{theater.valor}},00</b-card-text>

          <b-button  @click="addCartItem(theater)" variant="outline-secondary">comprar</b-button>
        </b-card>
      </div>
    </b-row>
  </div>
  ...
  computed: {
    filteredList() {
      return this.theaters.filter((theater) => {
        return theater.titulo.toLowerCase().includes(this.search.toLowerCase());
      });
    },
  },
  methods: {
