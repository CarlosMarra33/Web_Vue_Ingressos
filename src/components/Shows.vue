  ...
  <div class="shows">
    ...
    <b-row id="row">
      ...
      <div id="cards">
        ...
        >
          <b-card-text>{{show.descricao}}</b-card-text>
          <b-card-text>R${{show.valor}},00</b-card-text>

          <b-button  @click="addCartItem(show)" variant="outline-secondary">comprar</b-button>
        </b-card>
      </div>
    </b-row>
  ...
  computed: {
    filteredList() {
      return this.shows.filter((show) => {
        return show.titulo.toLowerCase().includes(this.search.toLowerCase());
      });
    },
  },
